...
graph TD
    ...
    subgraph Parallel Context Extraction Phase
        Coord -->|Handoff Request| CustAgent[Customer Agent]
        Coord -->|Handoff Request| OrderProdAgent[Order & Product Agent]
        Coord -->|Handoff Request| PayAgent[Payment Agent]
        Coord -->|Handoff Request| DelivAgent[Delivery Agent]
    end
...
| Agent Name | `input/` | `data/*.csv` | Internal State / Memory | `output/` | `logging/` |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Coordinator Agent** | **Read** | None | **Read/Write** | None | None |
| **Customer Agent** | None | **Read** (`orders`, `customers`) | **Read-only** | None | None |
| **Order & Product Agent** | None | **Read** (`orders`, `items`, `products`, `sellers`, `translation`) | **Read-only** | None | None |
| **Payment Agent** | None | **Read** (`items`, `payments`) | **Read-only** | None | None |
| **Delivery Agent** | None | **Read** (`orders`, `items`) | **Read-only** | None | None |
