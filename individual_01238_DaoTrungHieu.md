...
**Mã học viên (MHV)**: 01238  
**Nhóm**: B2_1  
**Vai trò trong dự án**: Data Analyst (Customer & Delivery)  

---

## 1. Công việc được phân công (Assigned Responsibilities)

- **Module phụ trách**: `Customer Agent` và `Delivery Agent`
- **Chi tiết nhiệm vụ**:
  - Viết hàm lấy `customer_unique_id` và quét lịch sử mua hàng.
  - Viết hàm xử lý Datetime để tính `delivery_variance_hours` và `handoff_variance_hours`.
  - Xác định người bán trễ hạn và nguyên nhân trễ hạn giao hàng.
